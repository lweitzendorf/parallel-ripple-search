...
template <class UserState> class AStarJHJSearch {
  ...
  unsigned int SearchStep() {
    ...
    } else // not goal
    {
      ...
      for (typename vector<Node *>::iterator successor = m_Successors.begin();
        ...
        // IFF in closed_so_far just skip
        if (closed_so_far.contains((*successor)->m_UserState.Hash()))
          continue;
        ...
        // check in some hash if the vertex was already closed
        ...
        // auto closedlist_result =
        //     closed_so_far.find((*successor)->m_UserState.Hash());
        // if (closedlist_result != closed_so_far.end())
        //   if (closedlist_result->second->g <= newg)
        //     FreeNode((*successor));
        ...
        //   auto clr = closedlist_result->second;

        //   clr->parent = (*successor)->parent;
        //   clr->g = (*successor)->g;
        //   clr->h = (*successor)->h;
        //   clr->f = (*successor)->f;
        ...
        //   m_OpenList.push_back(clr);

        //   // Remove closed node from closed list
        //   // m_ClosedList.erase(closedlist_result); // FIXME
        //   closed_so_far.erase(closedlist_result);
      ...
      closed_so_far.emplace(n->m_UserState.Hash());
...
private: // data
  ...
  unordered_set<int> closed_so_far;
